...
function getRcloneBin() {
  ...
  // Chemins d'installation classiques sous Windows
  const commonPaths = [
    'C:\\rclone\\rclone.exe',
...
function uploadToGoogleDrive(localFilePath, customRemoteFolder = null) {
  return new Promise((resolve, reject) => {
    ...
    // Obtenir le chemin absolu vers rclone.exe
...
// =========================================================================
// 2. CRON JOB : EXÉCUTION AUTOMATIQUE PAR INTERVALLE DE MINUTES
// =========================================================================
// =========================================================================
// 2. CRON JOB : PLANIFICATEUR AUTOMATIQUE CORRIGÉ
// =========================================================================
cron.schedule('* * * * *', async () => {
  try {
    // Requête SQL intelligente : PostgreSQL calcule lui-même :
    // 1. La date du jour
    // 2. La plage horaire (CURRENT_TIME BETWEEN heure_debut AND heure_fin)
    // 3. L'écart en minutes depuis le dernier backup (>= frequence_minutes)
    const query = `
      SELECT * FROM gbackup_schedule 
      WHERE actif = true 
      AND CURRENT_DATE BETWEEN date_debut AND date_fin
    ...
    for (const plan of rows) {
      ...
      // 1. On met à jour immédiatement le timestamp pour éviter les doublons
      await pool.query(
        `UPDATE gbackup_schedule SET dernier_backup = NOW() WHERE idbackup = $1`,
        [plan.idbackup]
      );
      ...
      // Nom du fichier avec timestamp horodaté
      const now = new Date();
      const timeStamp = now.getFullYear() + "-" + 
        String(now.getMonth() + 1).padStart(2, '0') + "-" + 
        String(now.getDate()).padStart(2, '0') + "_" + 
      ...
      exec(cmd, { env: { ...process.env, PGPASSWORD: dbPass } }, async (err, stdout, stderr) => {
        const isFileEmpty = fs.existsSync(filePath) && fs.statSync(filePath).size === 0;
        const status = (err || isFileEmpty) ? 'FAILED' : 'SUCCESS';
        const msg = err ? (stderr || err.message) : (isFileEmpty ? 'Fichier vide (0 octet)' : 'Sauvegarde réussie');
        ...
        if (status === 'SUCCESS') {
          console.log(`✅ [SUCCÈS] Sauvegarde créée : ${filePath}`);
        } else {
          console.error(`❌ [ÉCHEC] Sauvegarde : ${msg}`);
        }
      });
